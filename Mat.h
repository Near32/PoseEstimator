...
#define numeric_limits 0.00000001
...
inline Mat<T>::Mat(const Mat<T>& m, T oValue, int d_line, int d_column, int line, int column)
{
    ...
    {
        cout << "Attention : les parametres specifies ne permettent pas l'initialisation de la matrice : " << endl;
        cout << "m = " << m.getLine() << " " << m.getColumn() << endl;
        cout << d_line << " " << d_column << endl;
        cout << m_line << " " << m_column << endl;
    }
}
...
inline Mat<T>::Mat(const Mat<T>& m, int delLine[], int cLine, int delColumn[], int cColumn)
{
    ...
    {
        cout << "ERREUR : les parametres specifies ne permettent pas l'initialisation de la matrice." << endl;
    }
...
Mat<T> operator*(const Mat<T>& a, const Mat<T>& b)
{
    ...
    {
        cout << "Impossible d'operer la multiplication : mauvais formats de matrices.\n" << endl;
        cout << "Format m1 : " << a.getLine() << " x " << a.getColumn() << "\t Fromat m2 : " << b.getLine() << " x " << b.getColumn() << endl;
        exit(EXIT_FAILURE);        
        return Mat<T>(1,1);
    }
...
inline Mat<T> operator+(const Mat<T>& a, const Mat<T>& b)
{
    ...
    {
        cout << "Impossible d'operer l'addition : mauvais formats de matrices.\n" << endl;
        cout << "Format m1 : " << a.getLine() << " x " << a.getColumn() << "\t Fromat m2 : " << b.getLine() << " x " << b.getColumn() << endl;
        exit(EXIT_FAILURE);                
        return Mat<T>(1,1);
    }
...
inline Mat<T> operator-(const Mat<T>& a, const Mat<T>& b)
{
    if((a.getColumn() != b.getColumn()) || (a.getLine() != b.getLine()))
    {
        cout << "Impossible d'operer la soustraction : mauvais formats de matrices.\n" << endl;
        cout << "Format m1 : " << a.getLine() << " x " << a.getColumn() << "\t Fromat m2 : " << b.getLine() << " x " << b.getColumn() << endl;
        exit(EXIT_FAILURE);
        return Mat<T>(1,1);
    }
...
inline Mat<T> operator%(const Mat<T>& a, const Mat<T>& b)
{
    if((a.getColumn() != b.getColumn()) || (a.getLine() != b.getLine()))
    {
        cout << "Impossible d'operer la multiplication c_a_c : mauvais formats de matrices.\n" << endl;
        cout << "Format m1 : " << a.getLine() << " x " << a.getColumn() << "\t Fromat m2 : " << b.getLine() << " x " << b.getColumn() << endl;
        exit(EXIT_FAILURE);        
...
Mat<T> atan2(const Mat<T>& y, const Mat<T>& x)
{
    ...
    {
        cout << "ERREUR : atan : les matrices ne sont pas de memes dimensions." << endl;
    }
...
Mat<T> idmin(Mat<T> mat)
{
	Mat<T> id((T)(1.0/numeric_limits),3,1);	
	int n = mat.getLine();
	int m = mat.getColumn();
...
Mat<T> gaussTrans( Mat<T> C, int k)
{
    if(k >= 1 && k<= C.getLine())
    {
        ...
        {
          //throw("ERREUR : gauss transformation : le pivot fourni est nul.");
        }
    ...
    {
      //throw("ERREUR: mauvais parametre pour la transformation de gauss.");
    }
...
Mat<T> WielandHotellingTrans( Mat<T> A, Mat<T> eigv, T sigma, Mat<T> z)
{
	...
	if(eigv.getLine() == z.getLine() && eigv.getLine() == A.getLine() && A.getLine() == A.getColumn())
	{
        ...
        {
		cout << "PROBLEM : Wieland Hotelling Transformation...." << endl;
        }
	...
	if(A.getLine()==A.getColumn() && A.getLine()==q.getLine())
	{
		...
		if(temp != (T)0)
			r = (T) ((transpose(q)*A*q).get(1,1) / temp);
                ...
                {
			cout << "PROBLEM : RAYLEIGHT QUOTIENT / 2" << endl;
                }
	}
	else
        {
		cout << "PROBLEM : RAYLEIGHT QUOTIENT / 1" << endl;	
        }
    ...
    if( A_ != NULL && b_ != NULL)
    {
        ...
        switch(method)
        {
            ...
            for(int i=1;i<=n;i++)
            {
                ...
                {
#ifdef verbose
                    cout << "Singular Matrix : diag element : " << i << endl;
                    A.afficher();
                    //throw("Singular Matrix...");
#endif
                    A.set( numeric_limits, icol, icol);
                }
	     ...
	     Mat<T> Ai(I);
            ...
            *b_ = b;
            ...
            for(int i=1;i<=n;i++)
            {
                ...
                {
                	//throw("Singular Matrix...");
                	A.set( numeric_limits, irow, irow);
                }
...
public :
    ...
    LU(Mat<T> A, Mat<T>* L, Mat<T>* U)
    {
        ...
        if(L != NULL || U != NULL)
        {
           //throw("ERREUR : LU decomposition : mauvais pointeurs de matrices L et/ou U.");
        }
...
public :
    ...
    QR(Mat<T> A, Mat<T>* Q, Mat<T>* R)
    {
        ...
        if(Q != NULL || R != NULL)
        {
          //throw("ERREUR : QR decomposition : mauvais pointeurs de matrices L et/ou U.");
        }
...
{    
    ...
    for(int i=Si.getLine();i--;)
    {
    	for(int j=Si.getColumn();j--;)
    	{
			...
			{
                Si.set( (T)1.0/(numeric_limits), i+1,i+1);
			}            
		}		
	}
    }
...
Mat<T> computeHomoProj(Mat<T> X1, Mat<T> X2, int method = 1)
{
    ...
    {
        cout << "ERREUR : les nombres de points correspondants ne correspondent pas, ni pour une homographie, ni pour une projection." << endl;
    }
