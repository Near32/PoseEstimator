...
void TrackBallCamera::onKeyboard( const SDL_KeyboardEvent& kevent)
{
	...
	if( (kevent.type == SDL_KEYDOWN)&&(kevent.keysym.sym == SDLK_UP) )
	{
		centerX += delta;
		//std::cout << "CENTERX = " << centerX << std::endl;
	}
	
	if( (kevent.type == SDL_KEYDOWN)&&(kevent.keysym.sym == SDLK_DOWN) )
	{
		centerX -= delta;
		//std::cout << "CENTERX = " << centerX << std::endl;
	}
	
	if( (kevent.type == SDL_KEYDOWN)&&(kevent.keysym.sym == SDLK_RIGHT) )
	{
		centerY += delta;
		//std::cout << "CENTERY = " << centerY << std::endl;
	}
	
	if( (kevent.type == SDL_KEYDOWN)&&(kevent.keysym.sym == SDLK_LEFT) )
	{
		centerY -= delta;
		//std::cout << "CENTERY = " << centerY << std::endl;
	}
...
{	
	...
	glTranslated(centerZ,centerY,centerX);			
